...
static rt_s zz_test_encoding_get_info_do(enum rt_encoding encoding, const rt_char *encoding_enum)
{
	...
	if (!rt_char_append_un(encoding_info.code_page, 10, buffer, RT_CHAR_HALF_BIG_STRING_SIZE, &buffer_size)) goto error;
	...
	if (!rt_char_append(_R("\" "), 2, buffer, RT_CHAR_HALF_BIG_STRING_SIZE, &buffer_size)) goto error;
	if (!rt_char_append_un(encoding_info.code_unit_size, 10, buffer, RT_CHAR_HALF_BIG_STRING_SIZE, &buffer_size)) goto error;
	if (!rt_char_append_char(_R(','), buffer, RT_CHAR_HALF_BIG_STRING_SIZE, &buffer_size)) goto error;
	if (!rt_char_append_un(encoding_info.max_code_point_size, 10, buffer, RT_CHAR_HALF_BIG_STRING_SIZE, &buffer_size)) goto error;
