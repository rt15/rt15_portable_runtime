...
{
	rt_char current_directory[RT_FILE_PATH_SIZE];
	rt_un buffer_size;
	rt_s ret;

	if (!rt_console_write_string(_R("Current directory = \""))) goto error;
	if (!rt_file_path_get_current_dir(current_directory, RT_FILE_PATH_SIZE, &buffer_size)) goto error;
	if (!rt_console_write_string_with_size(current_directory, buffer_size)) goto error;
