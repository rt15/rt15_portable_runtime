...
static rt_s zz_test_get_application_data_dir()
{
	...
	if (!rt_console_write_string(_R("Application data directory = \""))) goto error;
	if (!rt_file_path_get_application_data_dir(_R("CodeBlocks"), application_data_dir, RT_FILE_PATH_SIZE, &application_data_dir_size)) goto error;
	if (!rt_console_write_string_with_size(application_data_dir, application_data_dir_size)) goto error;
	if (!rt_console_write_string(_R("\"\n"))) goto error;
...
static rt_s zz_test_get_home_dir()
{
	...
	if (!rt_console_write_string(_R("Home directory = \""))) goto error;
	if (!rt_file_path_get_home_dir(home_dir, RT_FILE_PATH_SIZE, &home_dir_size)) goto error;
	if (!rt_console_write_string_with_size(home_dir, home_dir_size)) goto error;
	if (!rt_console_write_string(_R("\"\n"))) goto error;
