...
static rt_s zz_manual_test_types_display_flags()
{
	...
	if (RT_UNLIKELY(!rt_console_write_string(_R("## Flags:\n")))) goto error;
#ifdef RT_DEFINE_DEBUG
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Debug build.\n")))) goto error;
#else
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Release build.\n")))) goto error;
#endif

#ifdef RT_DEFINE_32
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Architecture = 32 bits\n")))) goto error;
#else
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Architecture = 64 bits\n")))) goto error;
#endif

#ifdef RT_DEFINE_WINDOWS
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Operating system = Windows\n")))) goto error;
#else
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Operating system = Linux\n")))) goto error;
#endif

#ifdef RT_DEFINE_VC
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Compiler = Visual C++\n")))) goto error;
#else
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Compiler = GCC\n")))) goto error;
#endif

#ifdef RT_DEFINE_USE_CRT
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Use CRT = true\n")))) goto error;
#else
	if (RT_UNLIKELY(!rt_console_write_string(_R("  Use CRT = false\n")))) goto error;
#endif
...
#pragma GCC diagnostic ignored "-Wtype-limits"
	...
	if (rt_char_variable < 0) {
		if (RT_UNLIKELY(!rt_console_write_string(_R("rt_char is signed.\n"))))
			goto error;
	} else {
		if (RT_UNLIKELY(!rt_console_write_string(_R("rt_char is unsigned.\n"))))
			goto error;
	}
#ifdef RT_DEFINE_GCC
