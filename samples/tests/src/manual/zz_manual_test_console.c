...
static rt_s zz_manual_test_console_io_device()
{
	...
	if (!rt_io_device_create_from_console_input(&console_input)) {
		if (RT_UNLIKELY(!rt_console_write_string(_R("Failed to create console I/O device (normal in case of gdb).\n"))))
			goto error;
	} else {
		...
		if (RT_UNLIKELY(!rt_console_write_string(_R("Reading from console io device:\n")))) goto error;
		...
		if (RT_UNLIKELY(!rt_console_write_string(_R("Output into console io device: \"")))) goto error;
...
static rt_s zz_manual_test_console_read_file(const rt_char *test_resources_dir, const rt_char *file_name, enum rt_encoding encoding)
{
	...
	if (RT_UNLIKELY(!rt_console_write_string(encoding_info.name))) goto error;
	if (RT_UNLIKELY(!rt_console_write_string(_R(" file: OOOO")))) goto error;
	if (RT_UNLIKELY(!rt_console_write_string(buffer))) goto error;
	if (RT_UNLIKELY(!rt_console_write_string(_R("OOOO\n")))) goto error;
...
static rt_s zz_manual_test_read_line()
{
	...
	if (RT_UNLIKELY(!rt_console_write_string(_R("Please type a line:\n")))) goto error;
...
static rt_s zz_manual_test_read_char()
{
	...
	if (RT_UNLIKELY(!rt_console_write_string(_R("Please type a character:\n")))) goto error;
...
rt_s zz_manual_test_console()
{
	...
	if (RT_UNLIKELY(!rt_console_write_string(_R("Accented character in source file: OOOOÈOOOO\n")))) goto error;

	if (RT_UNLIKELY(!rt_console_write_string(_R("Writing nothing: OOOO")))) goto error;
