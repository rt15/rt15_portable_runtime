...
static rt_s zz_test_buffered_input_stream_prepare_file(const rt_char* file_path)
{
	...
	for (i = 0; i < 999; i++) {
		if (!output_stream->write(output_stream, zz_line, zz_line_size))
			goto error;
	}
	if (!output_stream->write(output_stream, zz_line, 13))
		goto error;
...
static rt_s zz_test_buffered_input_stream_do(const rt_char* file_path)
{
	...
	if (!zz_test_buffered_input_stream_prepare_file(file_path)) goto error;

	if (!rt_file_create(&file, file_path, RT_FILE_MODE_READ)) goto error;
	file_created = RT_TRUE;

	if (!rt_buffered_input_stream_create(&buffered_input_stream, &file.io_device.input_stream, stream_buffer, 80)) goto error;
	input_stream = &buffered_input_stream.input_stream;

	for (i = 0; i < 999; i++) {
		RT_MEMORY_SET(buffer, '1', 40);
		if (!input_stream->read(input_stream, buffer, 27, &bytes_read))
			goto error;
		if (bytes_read != 27)
			goto error;
		if (!rt_char8_equals(buffer, bytes_read, zz_line, zz_line_size))
			goto error;
	}

	/* Read the last cut off line. */
	RT_MEMORY_SET(buffer, '1', 40);
	if (!input_stream->read(input_stream, buffer, 15, &bytes_read))
		goto error;
	if (bytes_read != 13)
		goto error;
	if (!rt_char8_equals(buffer, bytes_read, zz_line, 13))
		goto error;
...
rt_s zz_test_buffered_input_stream()
{
	...
	if (!rt_char_append(_R("buffered_stream.txt"), 19, file_path, RT_FILE_PATH_SIZE, &file_path_size)) goto error;

	if (!zz_test_buffered_input_stream_do(file_path)) goto error;
