...
free:
	if (area) {
		if (RT_UNLIKELY(!heap->free(heap, (void**)&area) && ret))
			goto error;
...
static rt_s zz_test_alloc_if_needed(struct rt_heap *heap)
{
	...
	if (RT_UNLIKELY(!result)) goto error;
	...
	if (RT_UNLIKELY(heap_buffer_capacity != 100)) goto error;
	...
	if (RT_UNLIKELY(area != heap_buffer)) goto error;
	if (RT_UNLIKELY(heap_buffer_capacity != 100)) goto error;
	if (RT_UNLIKELY(heap_buffer != previous_heap_buffer)) goto error;
	for (i = 0; i < 75; i++)
		area[i] = 'a';
	...
	if (RT_UNLIKELY(!result)) goto error;
	if (RT_UNLIKELY(area != heap_buffer)) goto error;
	if (RT_UNLIKELY(heap_buffer_capacity != 150)) goto error;
	for (i = 0; i < 150; i++)
		area[i] = 'a';
...
static rt_s zz_test_runtime_heap(void)
{
	...
	if (RT_UNLIKELY(!zz_test_heap_do(&runtime_heap.heap))) goto error;
	if (RT_UNLIKELY(!zz_test_alloc_if_needed(&runtime_heap.heap))) goto error;
...
static rt_s zz_test_page_heap(void)
{
	...
	if (RT_UNLIKELY(!zz_test_heap_do(&page_heap.heap))) goto error;
	if (RT_UNLIKELY(!zz_test_alloc_if_needed(&page_heap.heap))) goto error;

	ret = RT_OK;
free:
	if (page_heap_created) {
		page_heap_created = RT_FALSE;
		if (RT_UNLIKELY(!page_heap.heap.close(&page_heap.heap) && ret))
			goto error;
	}
	return ret;
error:
...
rt_s zz_test_heap(void)
{
	...
	if (RT_UNLIKELY(!zz_test_runtime_heap())) goto error;
	if (RT_UNLIKELY(!zz_test_page_heap())) goto error;
