...
static rt_s zz_test_char_compare()
{
	...
	if (rt_char_compare(_R("a"), _R("È")) >= 0) goto error;
	if (rt_char_compare(_R("È"), _R("a")) <= 0) goto error;
...
static rt_s zz_test_char_hash()
{
	...
	if (!zz_test_char_hash_do(_R("Foo"), 17490737515057045975ul)) goto error;
	if (!zz_test_char_hash_do(_R("Bar"), 1603655411640082298ul)) goto error;
...
rt_s zz_test_char()
{
	...
	if (!zz_test_char_hash()) goto error;
