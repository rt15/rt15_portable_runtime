...
static rt_s zz_test_process_redirect_std_in_to_pipe(const rt_char* executable_path)
{
	...
	if (RT_UNLIKELY(!rt_temp_file_create(&file, _R("Zz"), temp_file, RT_FILE_PATH_SIZE, &buffer_size))) goto error;
	...
	if (RT_UNLIKELY(!rt_process_create_with_redirections(&process, RT_TRUE, RT_NULL, RT_NULL, input_io_device, &file.io_device, RT_NULL, application_path_and_args))) goto error;
	process_created = RT_TRUE;

	output_stream = &output_io_device->output_stream;
	if (RT_UNLIKELY(!output_stream->write(output_stream, "123\n", 4))) goto error;

	if (RT_UNLIKELY(!rt_process_join(&process))) goto error;
	if (RT_UNLIKELY(!rt_process_get_exit_code(&process, &exit_code))) goto error;
...
free:
	...
	if (delete_temp_file) {
		...
		if (RT_UNLIKELY(!rt_file_system_delete_file(temp_file) && ret))
			goto error;
	}
	return ret;
...
static rt_s zz_test_process_redirect_std_out_to_file(const rt_char* executable_path)
{
	...
	if (RT_UNLIKELY(!rt_process_create_with_redirections(&process, RT_TRUE, RT_NULL, RT_NULL, RT_NULL, &file.io_device, RT_NULL, application_path_and_args))) goto error;
	process_created = RT_TRUE;
...
static rt_s zz_test_process_redirect_std_err_to_file(const rt_char* executable_path)
{
	...
	if (RT_UNLIKELY(!rt_process_create_with_redirections(&process, RT_TRUE, RT_NULL, RT_NULL, RT_NULL, RT_NULL, &file.io_device, application_path_and_args))) goto error;
	process_created = RT_TRUE;
	...
	if (RT_UNLIKELY(!rt_file_system_get_file_size(temp_file, &file_size))) goto error;
	if (RT_UNLIKELY(file_size < 10)) goto error;
...
static rt_s zz_test_process_env(const rt_char* executable_path)
{
	...
	if (RT_UNLIKELY(!rt_env_vars_create(&env_vars))) goto error;
	env_vars_created = RT_TRUE;

	if (RT_UNLIKELY(!rt_env_vars_merge_env_var(&env_vars, _R("RT_PROCESS_VAR"), _R("VAR_VALUE")))) goto error;
	...
	if (RT_UNLIKELY(!rt_process_create_with_redirections(&process, RT_TRUE, RT_NULL, &env_vars, RT_NULL, output_io_device, RT_NULL, application_path_and_args))) goto error;
	process_created = RT_TRUE;
	...
	if (RT_UNLIKELY(!input_stream->read(input_stream, pipe_content, 256 - 1, &bytes_read))) goto error;
...
static rt_s zz_test_process_failed(void)
{
	...
	application_path_and_args[2] = RT_NULL;
	if (RT_UNLIKELY(rt_process_create(&process, RT_TRUE,  RT_NULL, RT_NULL, application_path_and_args))) goto error;
	if (RT_UNLIKELY(rt_process_create(&process, RT_FALSE, RT_NULL, RT_NULL, application_path_and_args))) goto error;

	/* Test wrong folder name. */
	application_path_and_args[0] = _R("ping");
	if (RT_UNLIKELY(rt_process_create(&process, RT_TRUE,  _R("Wrong, wrong directory"), RT_NULL, application_path_and_args))) goto error;
	if (RT_UNLIKELY(rt_process_create(&process, RT_FALSE, _R("Wrong, wrong directory"), RT_NULL, application_path_and_args))) goto error;
...
rt_s zz_test_process(void)
{
	...
	if (RT_UNLIKELY(!zz_test_process_redirect_std_in_to_pipe(executable_path))) goto error;
	if (RT_UNLIKELY(!zz_test_process_redirect_std_out_to_file(executable_path))) goto error;
	if (RT_UNLIKELY(!zz_test_process_redirect_std_err_to_file(executable_path))) goto error;
	if (RT_UNLIKELY(!zz_test_process_env(executable_path))) goto error;
	if (RT_UNLIKELY(!zz_test_process_failed())) goto error;
