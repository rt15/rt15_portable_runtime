...
static rt_s zz_test_file_mapping_read(const rt_char *file_path)
{
	...
	if (file_mapping_view_created) {
		if (RT_UNLIKELY(!rt_file_mapping_view_free(&file_mapping_view)))
			ret = RT_FAILED;
	}

	if (file_mapping_created) {
		if (RT_UNLIKELY(!rt_file_mapping_free(&file_mapping)))
			ret = RT_FAILED;
	}
...
static rt_s zz_test_file_mapping_write(const rt_char *lines_file_path, const rt_char *tmp_file_path)
{
	...
	if (RT_UNLIKELY(!rt_file_system_copy_file(lines_file_path, tmp_file_path, RT_TRUE))) goto end;
	if (RT_UNLIKELY(!zz_test_file_mapping_write_with_size(tmp_file_path, 80))) goto end;
	if (RT_UNLIKELY(!zz_test_file_mapping_check_file(tmp_file_path, 80))) goto end;

	if (RT_UNLIKELY(!rt_file_system_copy_file(lines_file_path, tmp_file_path, RT_TRUE))) goto end;
	if (RT_UNLIKELY(!zz_test_file_mapping_write_with_size(tmp_file_path, 40))) goto end;
	if (RT_UNLIKELY(!zz_test_file_mapping_check_file(tmp_file_path, 40))) goto end;
...
rt_s zz_test_file_mapping(void)
{
	...
	if (RT_UNLIKELY(!zz_get_test_resources_dir(lines_file_path, RT_FILE_PATH_SIZE, &lines_file_path_size))) goto end;
	if (RT_UNLIKELY(!rt_file_path_append_separator(lines_file_path, RT_FILE_PATH_SIZE, &lines_file_path_size))) goto end;
	if (RT_UNLIKELY(!rt_char_append(_R("lines.dat"), 9, lines_file_path, RT_FILE_PATH_SIZE, &lines_file_path_size))) goto end;

	if (RT_UNLIKELY(!zz_test_file_mapping_read(lines_file_path))) goto end;

	if (RT_UNLIKELY(!zz_get_tmp_dir(tmp_file_path, RT_FILE_PATH_SIZE, &tmp_file_path_size))) goto end;
	if (RT_UNLIKELY(!rt_file_path_append_separator(tmp_file_path, RT_FILE_PATH_SIZE, &tmp_file_path_size))) goto end;
	if (RT_UNLIKELY(!rt_char_append(_R("zz_test_file_mapping.dat"), 24, tmp_file_path, RT_FILE_PATH_SIZE, &tmp_file_path_size))) goto end;

	if (RT_UNLIKELY(!zz_test_file_mapping_write(lines_file_path, tmp_file_path))) goto end;
