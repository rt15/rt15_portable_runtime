...
static rt_un32 RT_STDCALL zz_test_select_server_callback(void *parameter)
{
	...
	if (RT_UNLIKELY(!rt_socket_create(&socket, RT_ADDRESS_FAMILY_IPV6, RT_SOCKET_TYPE_STREAM, RT_SOCKET_PROTOCOL_TCP, RT_FALSE, RT_FALSE))) goto error;
	...
	if (RT_UNLIKELY(!rt_socket_set_ipv6_boolean_option(&socket, RT_SOCKET_PROTOCOL_LEVEL_IPV6, RT_SOCKET_IPV6_OPTION_V6ONLY, RT_FALSE))) goto error;
	...
	if (RT_UNLIKELY(!rt_select_wait_for_ready_to_read(&socket, 0))) goto error;

	if (RT_UNLIKELY(!rt_socket_accept_connection(&socket, RT_FALSE, &accepted_socket, RT_NULL, RT_NULL))) goto error;
	...
	if (RT_UNLIKELY(bytes_sent != 14)) goto error;
...
static rt_s zz_test_select_client(void)
{
	...
	if (RT_UNLIKELY(!rt_socket_create(&socket, RT_ADDRESS_FAMILY_IPV4, RT_SOCKET_TYPE_STREAM, RT_SOCKET_PROTOCOL_TCP, RT_FALSE, RT_FALSE))) goto error;
	socket_created = RT_TRUE;
...
static rt_s zz_test_select_do(void)
{
	...
	if (RT_UNLIKELY(!rt_thread_create(&thread, &zz_test_select_server_callback, &server_parameter))) goto error;
	...
	if (RT_UNLIKELY(!zz_test_select_client())) goto error;
...
rt_s zz_test_select(void)
{
	...
	if (RT_UNLIKELY(!zz_test_select_do())) goto error;
