...
static rt_un32 RT_STDCALL zz_test_socket_server_callback(void *parameter)
{
	...
	if (RT_UNLIKELY(!rt_socket_create(&socket, server_parameter->address_family, RT_SOCKET_TYPE_STREAM, RT_SOCKET_PROTOCOL_TCP, RT_TRUE, RT_FALSE))) goto error;
	socket_created = RT_TRUE;

	/* Check REUSEADDR default value. */
	if (RT_UNLIKELY(!rt_socket_get_option(&socket, RT_SOCKET_PROTOCOL_LEVEL_SOCKET, RT_SOCKET_OPTION_REUSEADDR, &option, &option_size))) goto error;
	if (RT_UNLIKELY(option_size != sizeof(rt_n32))) goto error;
	if (RT_UNLIKELY(option)) goto error;

	/* Update REUSEADDR. */
	if (RT_UNLIKELY(!rt_socket_set_boolean_option(&socket, RT_SOCKET_PROTOCOL_LEVEL_SOCKET, RT_SOCKET_OPTION_REUSEADDR, RT_TRUE))) goto error;
	...
	if (server_parameter->address_family == RT_ADDRESS_FAMILY_IPV6) {
		if (RT_UNLIKELY(!rt_socket_set_ipv6_boolean_option(&socket, RT_SOCKET_PROTOCOL_LEVEL_IPV6, RT_SOCKET_IPV6_OPTION_V6ONLY, RT_TRUE))) goto error;
		if (RT_UNLIKELY(!rt_socket_get_ipv6_option(&socket, RT_SOCKET_PROTOCOL_LEVEL_IPV6, RT_SOCKET_IPV6_OPTION_V6ONLY, &option, &option_size))) goto error;
		if (RT_UNLIKELY(option_size != sizeof(rt_n32))) goto error;
		if (RT_UNLIKELY(!option)) goto error;
	}

	if (RT_UNLIKELY(!rt_socket_bind(&socket, ZZ_TEST_SOCKET_PORT_NUMBER))) goto error;
	if (RT_UNLIKELY(!rt_socket_listen(&socket))) goto error;

	if (RT_UNLIKELY(!rt_event_signal(&server_parameter->event))) goto error;

	if (RT_UNLIKELY(!rt_socket_accept_connection(&socket, RT_TRUE, &accepted_socket, RT_NULL, RT_NULL))) goto error;
	accepted_socket_created = RT_TRUE;
	shutdown_accepted_socket = RT_TRUE;

	message = "Hello";
	if (RT_UNLIKELY(!rt_socket_send(&accepted_socket, message, rt_char8_get_size(message), 0, &bytes_sent))) goto error;
	if (RT_UNLIKELY(bytes_sent != 5)) goto error;

	/* Uncomment next line to separate responses. */
	/* rt_sleep_sleep(1000); */

	message = ", world!";
	if (RT_UNLIKELY(!rt_socket_send(&accepted_socket, message, rt_char8_get_size(message) + 1, 0, &bytes_sent))) goto error;
	if (RT_UNLIKELY(bytes_sent != 9)) goto error;

	ret = RT_OK;
free:
	if (shutdown_accepted_socket) {
		shutdown_accepted_socket = RT_FALSE;
		if (RT_UNLIKELY(!rt_socket_shutdown(&accepted_socket, RT_SOCKET_SHUTDOWN_FLAG_BOTH) && ret))
			goto error;
	}
	if (accepted_socket_created) {
		accepted_socket_created = RT_FALSE;
		if (RT_UNLIKELY(!rt_socket_free(&accepted_socket) && ret))
			goto error;
	}
	if (socket_created) {
		socket_created = RT_FALSE;
		if (RT_UNLIKELY(!rt_socket_free(&socket) && ret))
			goto error;
	}
	return ret;
...
static rt_s zz_test_socket_client(enum rt_address_family address_family)
{
	...
	if (RT_UNLIKELY(!rt_socket_create(&socket, address_family, RT_SOCKET_TYPE_STREAM, RT_SOCKET_PROTOCOL_TCP, RT_TRUE, RT_FALSE))) goto error;
	socket_created = RT_TRUE;

	/* Check rt_io_device_is_console. */
	if (RT_UNLIKELY(!rt_io_device_is_console(&socket.io_device, &is_console))) goto error;
	...
	if (RT_UNLIKELY(!rt_socket_connect_with_socket_address(&socket, socket_address))) goto error;
	shutdown_socket = RT_TRUE;

	if (RT_UNLIKELY(!rt_socket_receive_all(&socket, buffer, RT_CHAR_BIG_STRING_SIZE, &received))) goto error;
	if (RT_UNLIKELY(received != rt_char8_get_size(buffer) + 1)) goto error;
	if (RT_UNLIKELY(!rt_char8_equals(buffer, received - 1, "Hello, world!", 13))) goto error;

	ret = RT_OK;
free:
	if (shutdown_socket) {
		shutdown_socket = RT_FALSE;
		if (RT_UNLIKELY(!rt_socket_shutdown(&socket, RT_SOCKET_SHUTDOWN_FLAG_BOTH) && ret))
			goto error;
	}
	if (socket_created) {
		socket_created = RT_FALSE;
...
static rt_s zz_test_socket_do(enum rt_address_family address_family)
{
	...
	if (RT_UNLIKELY(!rt_event_create(&server_parameter.event))) goto error;
	event_created = RT_TRUE;

	if (RT_UNLIKELY(!rt_thread_create(&thread, &zz_test_socket_server_callback, &server_parameter))) goto error;
	thread_created = RT_TRUE;

	if (RT_UNLIKELY(!rt_event_wait_for(&server_parameter.event))) goto error;

	/* Let some time for the server socket to be ready to accept connections. */
	rt_sleep_sleep(10);

	if (RT_UNLIKELY(!zz_test_socket_client(address_family))) goto error;
...
free:
	...
	if (event_created) {
		...
		if (RT_UNLIKELY(!rt_event_free(&server_parameter.event) && ret))
			goto error;
	}
	return ret;
...
rt_s zz_test_socket()
{
	...
	if (RT_UNLIKELY(!zz_test_socket_do(RT_ADDRESS_FAMILY_IPV4))) goto error;
	if (RT_UNLIKELY(!zz_test_socket_do(RT_ADDRESS_FAMILY_IPV6))) goto error;
