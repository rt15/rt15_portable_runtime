...
static rt_s zz_test_quick_sort_do(struct rt_heap *heap)
{
	...
	if (!rt_random_get_unsigned_integer_with_boundaries(2, 1000, &size)) goto error;
	if (!rt_array_create((void**)&array, size, sizeof(rt_un), 0, heap)) goto error;

	/* Fill the array with some random values. */
	for (i = 0; i < size; i++) {
		if (!rt_random_get_unsigned_integer_with_boundaries(0, 1000, &item))
			goto error;
		array[i] = item;
		check_sum += item;
	}
...
rt_s zz_test_quick_sort()
{
	...
	if (!zz_test_quick_sort_do(&runtime_heap.heap)) goto error;
