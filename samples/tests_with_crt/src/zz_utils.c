...
rt_s zz_stop_chrono(rt_char8 *identifier, struct rt_chrono *chrono, struct rt_output_stream *output_stream)
{
	...
	if (RT_UNLIKELY(!rt_char8_copy(identifier, buffer_size, buffer, 200))) goto error;
	if (RT_UNLIKELY(!rt_char8_append(": ", 2, buffer, 200, &buffer_size))) goto error;
	if (RT_UNLIKELY(!rt_char8_append_n(duration, 10, buffer, 200, &buffer_size))) goto error;
