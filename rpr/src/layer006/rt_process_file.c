...
free:
	if (output_file_created) {
		output_file_created = RT_FALSE;
		if (RT_UNLIKELY(!rt_io_device_free(&output_file.io_device) && ret))
			goto error;
	}
	if (input_file_created) {
		input_file_created = RT_FALSE;
		if (RT_UNLIKELY(!rt_io_device_free(&input_file.io_device) && ret))
			goto error;
	}
	return ret;
...
rt_s rt_process_file(const rt_char *file_path, rt_process_file_callback_t callback, void *context)
{
	...
	if (RT_UNLIKELY(!rt_process_file_with_output_file(file_path, tmp_file_path, callback, context))) goto error;
