...
rt_s rt_char8_append_hex(const rt_char8 *data, rt_un size, rt_un item_size,
...
{
	const rt_char8 *letters;
	rt_uchar8 byte;
	rt_char8 byte_chars[2];
	rt_un i, j;
	rt_s ret = RT_FAILED;
	...
	for (i = 0; i < size; i++) {
		if (i && separator_size) {
			if (RT_UNLIKELY(!rt_char8_append(separator, separator_size, buffer, buffer_capacity, buffer_size)))
				goto end;
		}

		if (prefix_size) {
			if (RT_UNLIKELY(!rt_char8_append(prefix, prefix_size, buffer, buffer_capacity, buffer_size)))
				goto end;
		}
		...
		for (j = 0; j < item_size; j++) {
			...
			if (RT_UNLIKELY(!rt_char8_append(byte_chars, 2, buffer, buffer_capacity, buffer_size)))
				goto end;
		}
	}
...
rt_s rt_char8_convert_to_f32(const rt_char8 *str, rt_f32 *result)
{
	...
	if (local_str[dot_index]) {
		...
		*result += decimal_part / rt_char8_base_get_power_of_ten32(rt_char8_get_size(&local_str[dot_index + 1]));
	} else {
...
rt_s rt_char8_convert_to_f32_with_size(const rt_char8 *str, rt_un str_size, rt_f32 *result)
{
	...
	if (dot_index != RT_TYPE_MAX_UN) {
		...
		/* Possible loss of data here. */
		*result = (rt_f32)integer_part;
		*result += decimal_part / rt_char8_base_get_power_of_ten32(local_str_size - dot_index - 1);
...
rt_s rt_char8_convert_to_f64(const rt_char8 *str, rt_f64 *result)
{
	...
	if (local_str[dot_index]) {
		...
		*result += decimal_part / rt_char8_base_get_power_of_ten64(rt_char8_get_size(&local_str[dot_index + 1]));
	} else {
		/* No dot, we convert the whole string as integer. */
...
rt_s rt_char8_convert_to_f64_with_size(const rt_char8 *str, rt_un str_size, rt_f64 *result)
{
	...
	if (dot_index != RT_TYPE_MAX_UN) {
		...
		/* Possible loss of data here. */
		*result = (rt_f64)integer_part;
		*result += decimal_part / rt_char8_base_get_power_of_ten64(local_str_size - dot_index - 1);
