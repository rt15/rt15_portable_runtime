...
#define RT_CHAR_FLOAT_TO_INT(value) ((value) >= 0 ? (rt_n)((value) + 0.5) : (rt_n)((value) - 0.5))
...
static rt_f32 rt_char_base_get_power_of_ten32(rt_n exponent)
{
	rt_un i;
	rt_f32 ret;
...
static rt_f64 rt_char_base_get_power_of_ten64(rt_n exponent)
{
	rt_un i;
	rt_f64 ret;
...
rt_s rt_char_append_f32(rt_f32 value, rt_un decimal_count, rt_char *buffer, rt_un buffer_capacity, rt_un *buffer_size)
{
	rt_f32 local_value;
	rt_char local_buffer[64];
	...
	if (!decimal_count) {
		...
		integer_part = RT_CHAR_FLOAT_TO_INT(local_value);
		if (RT_UNLIKELY(!rt_char_append_n(integer_part, 10, buffer, buffer_capacity, buffer_size)))
			goto error;
		...
		decimal_part = decimal_part * rt_char_base_get_power_of_ten32(decimal_count);
		...
		if (decimal_part > 1e-5f) {
			...
			local_buffer_size = 0;
			if (RT_UNLIKELY(!rt_char_append_un(RT_CHAR_FLOAT_TO_INT(decimal_part), 10, local_buffer, 64, &local_buffer_size)))
				goto error;
			...
			if (RT_UNLIKELY(!rt_char_append(local_buffer, local_buffer_size, buffer, buffer_capacity, buffer_size)))
				goto error;

			while (buffer[*buffer_size - 1] == _R('0')) {
				(*buffer_size)--;
			}
			if (buffer[*buffer_size - 1] == _R('.'))
				(*buffer_size)--;
...
rt_s rt_char_append_f64(rt_f64 value, rt_un decimal_count, rt_char *buffer, rt_un buffer_capacity, rt_un *buffer_size)
{
	rt_f64 local_value;
	rt_char local_buffer[64];
	rt_un local_buffer_size;
	...
	if (!decimal_count) {
		...
		decimal_part = decimal_part * rt_char_base_get_power_of_ten64(decimal_count);
...
rt_b rt_char_is_empty_or_blank(const rt_char *str)
{
	rt_char character;
	rt_un i;
	rt_b ret = RT_TRUE;
...
rt_b rt_char_is_empty_or_blank_with_size(const rt_char *str, rt_un str_size)
{
	rt_un i;
	rt_b ret = RT_TRUE;

	for (i = 0; i < str_size; i++) {
		if (!RT_CHAR_IS_BLANK(str[i])) {
			ret = RT_FALSE;
			break;
		}
	}
